...
def conv(image, kernel):
    ...
    ### YOUR CODE HERE
    # print(kernel.shape, pad_width0, pad_width1, Hi, Wi)
    ...
    for i in range(pad_width0, pad_width0 + Hi):
        for j in range(pad_width1, pad_width1 + Wi):
            ...
            # here can't use window *= flip_kernel! this will change padded_image content
            window = window * flip_kernel
...
def gaussian_kernel(size, sigma):
    ...
    for i in range(size):
        for j in range(size):
            exp_component = -((i - k) ** 2 + (j - k) ** 2) / (2 * sigma ** 2)
            kernel[i][j] = 1.0 / (2.0 * np.pi * sigma ** 2) * np.exp(exp_component)
    ### END YOUR CODE
...
def gradient(img):
    ...
    ### YOUR CODE HERE
    img_partial_x = partial_x(img)
    img_partial_y = partial_y(img)
    theta = (np.rad2deg(np.arctan2(img_partial_y, img_partial_x)) + 180) % 360
    G = np.sqrt(img_partial_x ** 2 + img_partial_y ** 2)
    ### END YOUR CODE
