...
class PCA(object):
    ...
    def _eigen_decomp(self, X):
        ...
        e_ind = np.argsort(e_vals)[::-1]
        e_vals = e_vals[e_ind]
        e_vecs = np.real(e_vecs[:, e_ind])
        # END YOUR CODE
    ...
    def transform(self, X, n_components):
        ...
        centerd_X = X - self.mean
        X_proj = centerd_X.dot(self.W_pca[:, :n_components])
        # END YOUR CODE
    ...
    def reconstruct(self, X_proj):
        ...
        inv_w = np.linalg.inv(self.W_pca)
        X = X_proj.dot(inv_w[:n_components, :])
        X += self.mean
        # END YOUR CODE
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        e_vals, e_vecs = scipy.linalg.eig(a=scatter_between, b=scatter_within)
        e_ind = np.argsort(e_vals)[::-1]
        # e_vals = e_vals[e_ind]
        e_vecs = e_vecs[:, e_ind]
        # END YOUR CODE
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            Si = Xi_centered.T.dot(Xi_centered)
            scatter_within += Si
            # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        #     yi_mask = (y == i)
        #     Xi = X[yi_mask]
        #     mu[i] = np.mean(Xi, axis=0).T
        # # scatter_between = np.cov(mu.T)

        # commented implmentation copy from https://github.com/Hugstar/Solutions-Stanford-cs131-Computer-Vision-Foundations-and-Application
        # mu = X.mean(axis=0)
        # temp_X = X.copy()
        # for i in np.unique(y):
        #     # YOUR CODE HERE
        ...
        for i in np.unique(y):
            ...
            tmp = mu_i - mu
            scatter_between += Ni * tmp.T.dot(tmp)
            # END YOUR CODE
