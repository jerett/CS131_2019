...
def compute_distances(X1, X2):
    ...
    X1X1 = np.sum(X1 ** 2, axis=1)
    X1X1 = X1X1.reshape(M, 1)
    X1X1 = np.repeat(X1X1, N, axis=1)
    # print(X1X1.shape)

    X2X2 = np.sum(X2 ** 2, axis=1)
    X2X2 = X2X2.reshape(1, N)
    X2X2 = np.repeat(X2X2, M, axis=0)
    # print(X2X2.shape)

    X1X2 = X1.dot(X2.T)
    # print(X1X2.shape)

    dists = X1X1 + X2X2 - 2 * X1X2
    dists = np.sqrt(dists)
    # END YOUR CODE
...
def split_folds(X_train, y_train, num_folds):
    ...
    X_pieces = np.array_split(X_train, num_folds, axis=0)
    y_pieces = np.array_split(y_train, num_folds, axis=0)

    for i in range(num_folds):
        X_temp = X_pieces.copy()
        y_temp = y_pieces.copy()

        X_vals[i] = X_temp[i]
        y_vals[i] = y_temp[i]
        del X_temp[i]
        del y_temp[i]
        X_trains[i] = np.concatenate(X_temp)
        y_trains[i] = np.concatenate(y_temp)
    # END YOUR CODE
