...
def hog_feature(image, pixel_per_cell=8):
    ...
    """
    ### YOUR CODE HERE
    hogFeature, hogImage = feature.hog(image, block_norm='L1', pixels_per_cell=(pixel_per_cell, pixel_per_cell), visualize=True)
    ### END YOUR CODE
...
def shift_heatmap(heatmap, mu):
    ...
    """
    ### YOUR CODE HERE
    heatmap= interpolation.shift(heatmap, shift=mu)
    ### END YOUR CODE
    return heatmap
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    for part_heatmap, sigma in zip(heatmaps, sigmas):
        heatmap += gaussian(part_heatmap, sigma)
    r, c = np.unravel_index(np.argmax(heatmap), heatmap.shape)
    ### END YOUR CODE
    return heatmap, r, c


def detect_multiple(image, response_map,  winH, winW):
    """
    Extra credit
    """
    ### YOUR CODE HERE
    # set reponse threshold
    face_conf = 30.0
    response_map_clone = response_map.copy()
    detected_faces = []
    while True:
        r, c = np.unravel_index(np.argmax(response_map_clone), response_map_clone.shape)
        score = response_map_clone[r, c]
        r -= winH // 2
        c -= winW // 2
        if score < face_conf:
            break
        ...
        response_map_clone[r: r + winH, c:c + winW] = 0
    ### END YOUR CODE
