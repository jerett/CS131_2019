...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        A = np.concatenate([Ay, Ax], axis=1)
        b = -It[y - w:y - w + window_size, x - w:x - w + window_size].reshape(-1, 1)
        [vy, vx] = np.linalg.inv(A.T.dot(A)).dot(A.T).dot(b).ravel()
...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        ### YOUR CODE HERE
        wy = Iy[y1 - w:y1 - w + window_size, x1 - w:x1 - w + window_size].reshape(-1, 1)
        wx = Ix[y1 - w:y1 - w + window_size, x1 - w:x1 - w + window_size].reshape(-1, 1)
        g0 = np.sum(wx**2)
        g1 = np.sum(wx * wy)
        g2 = np.sum(wy**2)
        G = np.array([[g0, g1],
        ...
        for k in range(num_iters):
            ...
            bk0 = Ik * wx
            bk1 = Ik * wy
            bk = np.array([[np.sum(bk0)],
                           [np.sum(bk1)]])
            vk = G_inv.dot(bk).ravel()
            ### END YOUR CODE
...
def compute_error(patch1, patch2):
    ...
    error = np.square(patch1_norm - patch2_norm)
    error = np.mean(error)
    ### END YOUR CODE
    return error
...
def IoU(bbox1, bbox2):
    ...
    xA = np.amax([x1, x2])
    yA = np.amax([y1, y2])
    xB = np.amin([x1 + w1, x2 + w2])
    yB = np.amin([y1 + h1, y2 + h2])
    union_x = np.amax([xB - xA, 0])
    union_y = np.amax([yB - yA, 0])
    union_area = union_x * union_y
    score = union_area / (w1 * h1 + w2 * h2 - union_area)
    # print(f'score: {score}')
    ### END YOUR CODE
